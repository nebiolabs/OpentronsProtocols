...
def run(ctx):
    ...
    #20 ul pipette and tips [LEFT MOUNT, SLOTS 4] 
    tips20 = [ctx.load_labware("opentrons_96_tiprack_20ul", 10, "Tips20")] 
    ...
    react = temp_mod.load_labware('vwr_96_aluminumblock_100ul', label = "Reaction")  #Change to 'vwr_96_aluminumblock_100ul' to use vwr 100 uL pcr plate on temp mod
    temp_mod.set_temperature(celsius=25)
  
    #load plates [SLOTS 5, 6, 8, 9]
    finalA = ctx.load_labware('nestedcemult1_96_wellplate_300ul', 7, "FinalPlateA") #Change to 'nestedcemult1_96_wellplate_300ul' to use nested CE plate on biorad 96 well plate
    finalB = ctx.load_labware('nestedcemult1_96_wellplate_300ul', 8, "FinalPlateB") # ''
    FinalPlates = [finalA, finalB]
    if exps >= 3:
        finalC = [ctx.load_labware('nestedcemult1_96_wellplate_300ul', 9, "FinalPlateC")] # ''
        FinalPlates = FinalPlates + finalC
        if exps >= 4:
            finalD = [ctx.load_labware('nestedcemult1_96_wellplate_300ul', 4, "FinalPlateD")] # ''
            FinalPlates = FinalPlates + finalD
            if exps >= 5:
                finalE = [ctx.load_labware('nestedcemult1_96_wellplate_300ul', 5, "FinalPlateE")] # ''
                FinalPlates = FinalPlates + finalE
                if exps >= 5:
                    finalF = [ctx.load_labware('nestedcemult1_96_wellplate_300ul', 6, "FinalPlateF")] # ''
                    FinalPlates = FinalPlates + finalF
    
    #water set up [SLOT 3]
    water = ctx.load_labware('nest_12_reservoir_15ml', 3,"Water") #Fill corresponding number of wells from L to R with water
    dilute = [water.wells()[n] for n in range(exps)]
    wash = ctx.load_labware('agilent_1_reservoir_290ml', 2)
    #liquid set up
    NTP = [react.rows()[0][n] for n in range(exps)] #Put NTP in first x rows
    Enzyme = [react.rows()[0][n+6] for n in range(exps)] #Put enzymes in x rows, starting from row 9
    FinalWells = []
    for n in range(exps):
    ...
    def m300transfer(vol, source, dest, mixrep, mixvol):
        ...
        #wash command
    def wash20():
        m20.mix(4, 15, wash.wells()[0], rate = 3)
        m20.blow_out(wash.wells()[0])

    #runplate1 functions MIX AFTER STEP AND WASH TIPS
    def taketp(rxn,tp):
    ...
    #dilute with water
    for m in range(exps):
        m300.distribute(100, dilute[m], [FinalWells[m][n].top() for n in range(len(FinalWells[m]))], new_tip = 'once', disposal_volume = 5)
